...
class VersionResolverTest {
    ...
    void testBump(String bump, String expectedVersion) throws IOException, InterruptedException {
        ...
        when(versionsProvider.listVersions())
                .thenReturn(new TreeSet<>(Set.of(Version.parse("1.2.3"), Version.parse("1.3.0"))));

        // act
        Version result = versionResolver.resolve(bump);

        // assert
        assertEquals(Version.parse(expectedVersion), result);
    }
    ...
    void testExplicitVersionHappyFlow() throws IOException, InterruptedException {
        ...
        // act
        Version result = versionResolver.resolve("1.3.1");

        // assert
        assertEquals(Version.parse("1.3.1"), result);
    }
